<template>
    <div class="chooseCity">
      <Select v-model="model1" style="min-width:80px;max-width:90px;" @on-change="deleteModel(1)">
        <Option v-for="item in firstFloorDatas" :value="item.id" :key="item.id">{{ item.name }}</Option>
      </Select>
      <Select v-model="model2" style="min-width:80px;max-width: 90px; margin-left:5px;" v-if="showSecond" @on-change="deleteModel(2)">
        <Option v-for="item in secondFloorDatas" :value="item.id" :key="item.id">{{ item.name }}</Option>
      </Select>
      <Select v-model="model3" style="min-width:90px;max-width: 100px; margin-left:5px;" v-if="showThird" >
...
<script>
  ...
  var cityDatas = require('../../assets/js/cityDatas')
    export default {
        name: "chooseCity",
        data() {
          return {
...
<style >
  .chooseCity {height:32px;float: right;}
</style>
